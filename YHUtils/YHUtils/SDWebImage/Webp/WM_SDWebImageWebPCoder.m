...
#import "WM_SDWebImageCoderHelper.h"
...
@implementation WM_SDWebImageWebPCoder {
    WebPIDecoder *_idec;
}
...
+ (instancetype)sharedCoder {
    static WM_SDWebImageWebPCoder *coder;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        coder = [[WM_SDWebImageWebPCoder alloc] init];
    });
    return coder;
}
...
- (BOOL)canDecodeFromData:(nullable NSData *)data {
    return ([NSData wm_sd_imageFormatForImageData:data] == WM_SDImageFormatWebP);
}
...
- (UIImage *)decodedImageWithData:(NSData *)data {
    ...
    CGContextRef canvas = CGBitmapContextCreate(NULL, canvasWidth, canvasHeight, 8, 0, WM_SDCGColorSpaceGetDeviceRGB(), bitmapInfo);
    if (!canvas) {
    ...
    if (!(flags & ANIMATION_FLAG)) {
        ...
        UIImage *staticImage = [self wm_rawWebpImageWithData:webpData];
        if (staticImage) {
    ...
    NSMutableArray<WM_SDWebImageFrame *> *frames = [NSMutableArray array];
    
    do {
        @autoreleasepool {
            UIImage *image = [self wm_drawnWebpImageWithCanvas:canvas iterator:iter];
            if (!image) {
                continue;
            }
            ...
            WM_SDWebImageFrame *frame = [WM_SDWebImageFrame frameWithImage:image duration:duration / 1000.f];
    ...
    UIImage *animatedImage = [WM_SDWebImageCoderHelper animatedImageWithFrames:frames];
    animatedImage.wm_imageLoopCount = loopCount;
...
- (UIImage *)incrementallyDecodedImageWithData:(NSData *)data finished:(BOOL)finished {
    ...
    if (width + height > 0 && last_y > 0 && height >= last_y) {
        ...
        CGColorSpaceRef colorSpaceRef = WM_SDCGColorSpaceGetDeviceRGB();
        ...
        CGContextRef canvas = CGBitmapContextCreate(NULL, width, height, 8, 0, WM_SDCGColorSpaceGetDeviceRGB(), bitmapInfo);
        if (!canvas) {
...
- (nullable UIImage *)wm_drawnWebpImageWithCanvas:(CGContextRef)canvas iterator:(WebPIterator)iter {
    UIImage *image = [self wm_rawWebpImageWithData:iter.fragment];
    if (!image) {
        return nil;
    }
...
- (nullable UIImage *)wm_rawWebpImageWithData:(WebPData)webpData {
...
- (BOOL)canEncodeToFormat:(WM_SDImageFormat)format {
    return (format == WM_SDImageFormatWebP);
}
...
- (NSData *)encodedDataWithImage:(UIImage *)image format:(WM_SDImageFormat)format {
    ...
    NSArray<WM_SDWebImageFrame *> *frames = [WM_SDWebImageCoderHelper framesFromAnimatedImage:image];
    if (frames.count == 0) {
        // for static single webp image
        data = [self wm_encodedWebpDataWithImage:image];
    } else {
        // for animated webp image
        ...
        for (size_t i = 0; i < frames.count; i++) {
            WM_SDWebImageFrame *currentFrame = frames[i];
            NSData *webpData = [self wm_encodedWebpDataWithImage:currentFrame.image];
        ...
        int loopCount = (int)image.wm_imageLoopCount;
...
- (nullable NSData *)wm_encodedWebpDataWithImage:(nullable UIImage *)image {
    if (!image) {
        return nil;
    }
    ...
    if (byteOrderNormal && ((alphaInfo == kCGImageAlphaNone) || (alphaInfo == kCGImageAlphaLast))) {
        ...
        vImage_CGImageFormat destFormat = {
            ...
            .colorSpace = WM_SDCGColorSpaceGetDeviceRGB(),
