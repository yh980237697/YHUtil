...
@implementation YH_SDWebImageWebPCoder {
    WebPIDecoder *_idec;
}
...
+ (instancetype)sharedCoder {
    static YH_SDWebImageWebPCoder *coder;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        coder = [[YH_SDWebImageWebPCoder alloc] init];
    });
    return coder;
}
...
- (BOOL)canDecodeFromData:(nullable NSData *)data {
    return ([NSData yh_sd_imageFormatForImageData:data] == YH_SDImageFormatWebP);
}
...
- (UIImage *)decodedImageWithData:(NSData *)data {
    ...
    CGContextRef canvas = CGBitmapContextCreate(NULL, canvasWidth, canvasHeight, 8, 0, YH_SDCGColorSpaceGetDeviceRGB(), bitmapInfo);
    if (!canvas) {
    ...
    if (!(flags & ANIMATION_FLAG)) {
        ...
        UIImage *staticImage = [self yh_rawWebpImageWithData:webpData];
        if (staticImage) {
    ...
    do {
        @autoreleasepool {
            UIImage *image = [self yh_drawnWebpImageWithCanvas:canvas iterator:iter];
            if (!image) {
                continue;
            }
            ...
            YH_SDWebImageFrame *frame = [YH_SDWebImageFrame frameWithImage:image duration:duration / 1000.f];
    ...
    UIImage *animatedImage = [YH_SDWebImageCoderHelper animatedImageWithFrames:frames];
...
- (UIImage *)incrementallyDecodedImageWithData:(NSData *)data finished:(BOOL)finished {
    ...
    if (width + height > 0 && last_y > 0 && height >= last_y) {
        ...
        CGContextRef canvas = CGBitmapContextCreate(NULL, width, height, 8, 0, YH_SDCGColorSpaceGetDeviceRGB(), bitmapInfo);
        if (!canvas) {
...
- (nullable UIImage *)yh_drawnWebpImageWithCanvas:(CGContextRef)canvas iterator:(WebPIterator)iter {
    UIImage *image = [self yh_rawWebpImageWithData:iter.fragment];
    if (!image) {
        return nil;
    }
...
- (nullable UIImage *)yh_rawWebpImageWithData:(WebPData)webpData {
...
- (BOOL)canEncodeToFormat:(YH_SDImageFormat)format {
    return (format == YH_SDImageFormatWebP);
}
...
- (NSData *)encodedDataWithImage:(UIImage *)image format:(YH_SDImageFormat)format {
    ...
    if (frames.count == 0) {
        // for static single webp image
        data = [self yh_encodedWebpDataWithImage:image];
    } else {
        // for animated webp image
        ...
        for (size_t i = 0; i < frames.count; i++) {
            YH_SDWebImageFrame *currentFrame = frames[i];
            NSData *webpData = [self yh_encodedWebpDataWithImage:currentFrame.image];
        ...
        int loopCount = (int)image.yh_imageLoopCount;
...
- (nullable NSData *)yh_encodedWebpDataWithImage:(nullable UIImage *)image {
    if (!image) {
        return nil;
    }
    ...
    if (byteOrderNormal && ((alphaInfo == kCGImageAlphaNone) || (alphaInfo == kCGImageAlphaLast))) {
        ...
        vImage_CGImageFormat destFormat = {
            ...
            .colorSpace = YH_SDCGColorSpaceGetDeviceRGB(),
