...
@implementation WM_SDWebImageGIFCoder

+ (instancetype)sharedCoder {
    static WM_SDWebImageGIFCoder *coder;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        coder = [[WM_SDWebImageGIFCoder alloc] init];
    });
    return coder;
}
...
- (BOOL)canDecodeFromData:(nullable NSData *)data {
    return ([NSData wm_sd_imageFormatForImageData:data] == WM_SDImageFormatGIF);
}
...
- (UIImage *)decodedImageWithData:(NSData *)data {
    ...
    if (count <= 1) {
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            float duration = [self wm_frameDurationAtIndex:i source:source];
            UIImage *image = [[UIImage alloc] initWithCGImage:imageRef];
            CGImageRelease(imageRef);
            
            WM_SDWebImageFrame *frame = [WM_SDWebImageFrame frameWithImage:image duration:duration];
        ...
        animatedImage = [WM_SDWebImageCoderHelper animatedImageWithFrames:frames];
...
- (BOOL)canEncodeToFormat:(WM_SDImageFormat)format {
    return (format == WM_SDImageFormatGIF);
}
...
- (NSData *)encodedDataWithImage:(UIImage *)image format:(WM_SDImageFormat)format {
    ...
    if (format != WM_SDImageFormatGIF) {
        return nil;
    }
    
    NSMutableData *imageData = [NSMutableData data];
    CFStringRef imageUTType = [NSData wm_UTTypeFromSDImageFormat:WM_SDImageFormatGIF];
    ...
    if (frames.count == 0) {
        ...
        NSUInteger loopCount = image.wm_imageLoopCount;
        ...
        for (size_t i = 0; i < frames.count; i++) {
            WM_SDWebImageFrame *frame = frames[i];
